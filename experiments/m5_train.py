...
def build_init(arm, cfg, device):
    ...
    moments = None
    if arm in ("hybrid", "hybrid_rs", "subclone_rs_act", "hybrid_rs_act"):
    ...
    if arm in ("subclone", "subclone_rs", "subclone_iso", "hybrid", "hybrid_rs",
        ...
        # "_act" arms score V/O and MLP wires by activation energy (the Σ the
        # compensation reads) instead of weight norms: the information-matched control.
        sel = subclone.select_indices(WL, moments=moments if arm.endswith("_act") else None, **kw)
        if arm.startswith("subclone"):
            rescale = {"subclone_rs": "full", "subclone_rs_act": "full", "subclone_iso": "smart"}.get(arm, False)
            W = subclone.subclone_weights(WL, res_idx, sel=sel, rescale=rescale, **kw)
            del moments
        else:
            W = subclone.hybrid_weights(WL, res_idx, moments, sel=sel, **kw)
            del moments
            if arm in ("hybrid_rs", "hybrid_rs_act"):  # reference-recipe scale on the LN-fronted reads
