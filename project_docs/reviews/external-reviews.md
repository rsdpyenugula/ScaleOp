...
| R3.3 | Subcloning implementation fidelity vs the reference recipe | ✅ Resolved (2026-07-31) | Verified vs arXiv:2312.09299 (full text). Differences: (a) reference scores by activation magnitudes, ours by weight norms for heads/MLP + activation variance for residual lanes — disclosed, baseline labeled "subcloning-style"; (b) reference removes MIDDLE blocks, ours even-stride — disclosed; (c) reference rescales weights by √(d/d′) — **tested: blanket rescale collapses zero-shot to ~62k (vs 11.5k without) because LayerNorm already renormalizes most read paths**; the variance argument only holds for O/MLP_DOWN (no norm between cut and read) — exactly the two matrices our LS compensation re-fits. Isotropic-rescale-on-those-two ("subclone_iso") queued as the middle ablation rung: selection → +isotropic scale → +LS compensation. |
