...
is provably destructive because it breaks the architecture's load-bearing wiring,
and after the best-fit linear operator the weight residuals are indistinguishable
from noise. The value of a conversion therefore lives in \emph{initialization}, not
in any learnable zero-shot weight correction. Decomposing that initialization into
two independent levers --- least-squares \emph{compensation} (what the network
computes) and variance-preserving \emph{rescale} (the scale the optimizer sees) ---
